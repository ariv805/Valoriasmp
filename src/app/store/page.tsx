...
import { RANKS, WHATSAPP_NUMBER } from '@/lib/constants';
...
export default function StorePage() {
  ...
  const [purchaseType, setPurchaseType] = useState<'rank' | 'coins' | 'money' | 'skills'>('rank');
...
Mohon proses pembayaran saya. Terima kasih!`;
      ...
      message = `Halo, saya ingin membeli ${purchaseType}

📦 Detail Pembelian:
- Item: ${purchaseType.charAt(0).toUpperCase() + purchaseType.slice(1)}
- Nama Player: ${playerNameFormatted}
- Platform: ${platform.toUpperCase()}
...
Mohon informasi lebih lanjut. Terima kasih!`;
  ...
  const openPurchaseDialog = (rank: typeof RANKS[0]) => {
    ...
    <PageWrapper>
      <div className="min-h-screen px-4 py-20">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-gray-400 text-center mb-8 text-sm">Tingkatkan pengalaman bermain dengan rank eksklusif</p>

          {/* Store Tabs */}
          <Tabs defaultValue="ranks" className="w-full">
            <TabsList className="grid w-full max-w-md mx-auto grid-cols-4 mb-6 glass h-10">
                  ...
                  >
                    ...
                    <Card className={`glass border-0 h-full hover:scale-[1.02] transition-all duration-300 ${rank.top ? 'glow-gold' : rank.popular ? 'glow-purple' : ''}`}>
                      ...
                      <CardContent className="space-y-3">
                        ...
                        <Button 
                          onClick={() => openPurchaseDialog(rank)}
              ...
              <TabsContent key={tab} value={tab}>
                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 max-w-3xl mx-auto">
                  {[1, 2, 3, 4].map((i) => (
                    ...
                    >
                      <Card className="glass border-0 text-center hover:scale-[1.02] transition-transform">
                        <CardHeader className="py-4">
                          <div className="w-12 h-12 rounded-full bg-emerald-500/20 flex items-center justify-center mx-auto mb-3">
                            {tab === 'coins' && <Coins className="w-6 h-6 text-emerald-400" />}
                            {tab === 'money' && <Package className="w-6 h-6 text-emerald-400" />}
                            {tab === 'skills' && <Zap className="w-6 h-6 text-emerald-400" />}
                          </div>
                          <CardTitle className="text-base">{tab === 'coins' ? 'Coins' : tab === 'money' ? 'Money' : 'Skills'} Pack {i}</CardTitle>
                        </CardHeader>
                        <CardContent className="pt-0">
                          <p className="text-lg font-bold text-emerald-400 mb-3">Rp {i * 25000}</p>
                          <Button 
                            onClick={() => {
                              setPurchaseType(tab as 'coins' | 'money' | 'skills');
                              setSelectedRank(null);
                              setPurchaseDialogOpen(true);
                            }}
                            className="w-full bg-emerald-500 hover:bg-emerald-600 text-sm h-8"
                          >
                            Beli Sekarang
                          </Button>
                        </CardContent>
        ...
        <DialogContent className="glass border-0 max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-lg font-minecraft text-amber-400">
              {purchaseType === 'rank' && selectedRank ? `Beli Rank ${selectedRank.name}` : `Beli ${purchaseType.charAt(0).toUpperCase() + purchaseType.slice(1)}`}
            </DialogTitle>
